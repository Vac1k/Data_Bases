...
CREATE VIEW shots_per_country AS
...
CREATE VIEW player_per_ligue AS
...
CREATE VIEW goal_per_player AS
SELECT name, goals
...
with conn:
    ...
    cur.execute('DROP VIEW IF EXISTS shots_per_country')
    cur.execute(query_1)
    cur.execute('SELECT * FROM shots_per_country;')
    ...
    cur.execute('DROP VIEW IF EXISTS player_per_ligue')
    cur.execute(query_2)
    cur.execute('SELECT * FROM player_per_ligue LIMIT 5;')
    names = []
    total = []
    ...
    cur.execute('DROP VIEW IF EXISTS goal_per_player')
    cur.execute(query_3)
    cur.execute('SELECT * FROM goal_per_player LIMIT 7;')
    names = []
    total = []

    for i, row in enumerate(cur):
        names.append(f'{i}.{row[0]}')
        total.append(row[1])
    ...
    for name, t in zip(names, total):
        graph_ax.annotate(t, xy=(name, t), color=mark_color,
